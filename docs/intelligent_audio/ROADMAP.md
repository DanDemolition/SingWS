...
Date: September 12, 2026 — revised September 15, 2026 (Apple Silicon / macOS 15+ only)

Companion hookup diagram: `SingWS_Mixer_Hookup_Guide.png`

## Purpose

This document preserves the design discussed for gradually adding intelligent music transitions, live microphone awareness, and eventually software vocal effects to SingWS. The overriding goal is reliability during live karaoke. SingWS 2.0 targets Apple Silicon on macOS 15 or later only. Every feature must fail safely without interrupting playback.

## Platform decision (SingWS 2.0)

- **Supported:** Apple Silicon (arm64) Macs running macOS 15 or later. One arm64 build; no universal2 or x86_64 packaging.
- **Performance floor:** base M1. All CPU, memory, and latency budgets are measured there.
- **SingWS 1.x** remains the release for Intel Macs and older macOS, and receives bug fixes for roughly one year, until 2.0 is fully stable. 2.0 work must not be backported into 1.x beyond critical fixes.
- **Platform APIs first:** prefer built-in macOS frameworks over bundled third-party runtimes where they meet the need:
  - **SoundAnalysis** (built-in sound classifier: singing, speech, music, applause, silence, and more) is the first candidate for audio classification, running on the Neural Engine with no bundled model.
  - **Core Audio process taps** (macOS 14.2+) are the first candidate for capturing SingWS program audio without adding work to the Python playback path.
  - **AVAudioEngine and Apple Audio Units** (reverb, delay, and related effects) in a separate native **Swift helper process** form the vocal-effects engine. Python never runs hard-real-time effects processing.
  - **Core ML** is the default path for any custom model; ONNX Runtime is a fallback only.
- Swift helpers are built, signed, and notarized as part of the SingWS app bundle and controlled by SingWS over a small local control channel. A helper crash must never interrupt playback.
...
First candidate: Apple's built-in **SoundAnalysis** classifier (`SNClassifySoundRequest` with the system classifier), run from a native Swift analysis helper or via PyObjC in a worker process.

- Input: mono audio from a Core Audio process tap or bounded copy of program audio, plus optional USB mic channels
- Analysis window: approximately 1 second, with overlap configurable
- Runs on the Neural Engine/CPU chosen by the system; no model file is bundled
- Fallback: a YAMNet-derived classifier converted to **Core ML**; ONNX Runtime only if Core ML conversion fails

The SingWS decision layer maps classifier output into should report probabilities for:
...
The classifier should not decide transitions by itself. Its output should be combined with signal measurements, file metadata, and lyric state.
...
- Packaged arm64 build on the minimum supported macOS 15 release and the current release
...
- Observer mode has completed multiple full shows on Apple Silicon (including an M1-class machine).
...
Implement Phase 0 and the observer-mode data model before adding a classifier or USB capture. Specifically:
...
- Target is Apple Silicon on macOS 15+ only; budgets are measured on a base M1. Do not add Intel or pre-15 compatibility code.
...
```text
You are working in the SingWS repository. We are beginning a multi-stage Intelligent Audio project for reliable karaoke-to-BGM transitions and later optional vocal effects. SingWS 2.0 targets Apple Silicon on macOS 15+ only; Intel and older macOS stay on 1.x. This session is architecture discovery only.
...
- logging, crash reporting, tests, and arm64 packaging, signing, and notarization
...
5. Risks specific to M1-floor performance, the Python GIL, device switching, shutdown, and packaged builds.
6. A proposed typed event/state model without implementing it.
7. A dependency recommendation comparing Apple SoundAnalysis, Core Audio process taps, Core ML, ONNX Runtime, Essentia, and existing dependencies, including packaging impact.
...
Run focused tests plus the existing relevant suite. Clearly separate tests actually run from tests requiring real Apple Silicon hardware, a mixer, or a live show.
...
- Bound memory and CPU usage, measured against a base M1.
...
Run all relevant tests and document what still needs live-show observation on Apple Silicon.
...
```text
Perform a bounded feasibility spike for lightweight audio classification in SingWS 2.0 (Apple Silicon, macOS 15+ only). Read all `docs/intelligent_audio/` material and inspect the arm64 build, signing, and notarization configuration.

Evaluate in this order and do not wire anything into live playback:
1. Apple SoundAnalysis built-in classifier (`SNClassifySoundRequest`), from a small Swift helper and, for comparison, via PyObjC in a worker process.
2. Core Audio process taps (macOS 14.2+) as the source of program audio, versus a bounded copy from the existing decoder.
3. Only if SoundAnalysis fails the needs: a YAMNet-derived model converted to Core ML; ONNX Runtime CPU as a last resort.

Evaluate and document:
- which built-in classes map usefully to active vocal, speech, instrumental music, silence, applause/crowd, and uncertain
- latency, cold start, average and worst-case analysis time, CPU, memory, and energy impact on a base M1
- behavior on silence, singing, speech, instrumental music, applause, and noisy mixtures, including false confidence
- how results reach SingWS (helper IPC design, message format, bounded queue, helper crash/restart)
- microphone permission (TCC) prompts and usage strings for the app and any helper needed for process taps and microphone input, and user prompts
- bundle, signing, hardened runtime, and notarization impact of any helper
- license and provenance for any non-Apple model
- whether deterministic features or Essentia are better for any output

Do not claim measurements that were not run on real Apple Silicon hardware; provide repeatable benchmark commands. Propose explicit M1 budgets for CPU, memory, latency, queue depth, and startup impact. Recommend Go/No-Go per option and a fallback.

Place spike code under a clearly marked experimental location that normal SingWS builds do not load. Update `docs/intelligent_audio/MODEL_EVALUATION.md` and `HANDOFF.md`. Do not enable classifier-driven decisions.
```

**Gate:** Proceed only if the chosen option packages, signs, and notarizes as arm64 and meets the M1 budget on real hardware.
...
```text
Integrate the approved classifier (SoundAnalysis helper or Core ML fallback) into SingWS Observer Mode only. Before coding, read `docs/intelligent_audio/MODEL_EVALUATION.md`, `ARCHITECTURE.md`, and `HANDOFF.md`. Stop and report a blocker if the evaluation does not contain an explicit Go decision and verified packaging plan.

Requirements:
- Feed the classifier program audio via the approved source (Core Audio process tap or bounded nonblocking copy), never from inside the Python playback callback beyond the approved minimal handoff.
...
Run packaging/build checks available in the environment and list actual versus pending verification on real Apple Silicon hardware.
...
- simultaneous playback and capture through the same USB device (recommended default, avoiding clock drift)
- whether capture belongs in the Swift audio helper rather than Python
- CoreAudio device enumeration and stable identity
- channel mapping and sample-rate conversion
- aggregate-device, clock-drift, and device-switch risks on macOS 15+
- input loss/reconnect behavior
- microphone permission (TCC) prompts and usage strings for the app and any helper
- meters/calibration
...
- required unit, replay, integration, Apple Silicon hardware, and live-show tests
...
```text
Design the SingWS Vocal Effects subsystem as a native Swift helper process using AVAudioEngine and Apple Audio Units, controlled by SingWS over a local control channel. Python must never run the effects callback. This is separate from Intelligent Transition analysis and must never share its worker queue, latency assumptions, or failure path. Read all `docs/intelligent_audio/`, current audio I/O, and the current supported mixer workflows.
...
- arm64 helper build, signing, hardened runtime, and notarization inside the SingWS bundle
- round-trip latency of the wet return versus the dry mic (reverb/slapback tolerate it; chorus/doubling may comb-filter and must be validated)
- helper crash/restart without affecting karaoke or BGM playback
...
Do not implement effects yet. Produce `docs/intelligent_audio/VOCAL_EFFECTS_ARCHITECTURE.md` with a signal-flow diagram, callback contract, evaluation of built-in Apple Audio Units versus any third-party DSP, buffer strategy, latency budget, safety invariants, staged implementation plan, and test plan. Include a minimal first milestone that passes audio through with unity gain and then adds one wet-only effect.
...
- arm64 package contents, Swift helpers, signing, and notarization are correct
...
- SingWS 2.0 is Apple Silicon, macOS 15+ only; 1.x covers Intel/older macOS with about a year of bug fixes.
- Platform APIs first: SoundAnalysis for classification, Core Audio process taps for program audio, Core ML for any custom model.
- Vocal effects run in a native Swift helper using AVAudioEngine/Audio Units, never in Python.
