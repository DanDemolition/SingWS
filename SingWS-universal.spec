...
# GStreamer has been removed from SingWS. The universal build no longer copies
# GStreamer (framework plugins, typelibs, scanner, core dylibs) — the
# single largest thing in the bundle (~315 MiB). `gi` is in `excludes` below
# so PyInstaller cannot pull GStreamer back in.
...
app = BUNDLE(
    ...
    info_plist={
        ...
        'CFBundleShortVersionString': '0.4.2.9',
        'CFBundleVersion': '0.4.2.9',
