"""libmpv/Qt background-music recovery engine.
...
Audio path: one isolated bundled-libmpv job per deck decodes stereo PCM into
...
from media_helpers import probe_duration_seconds, match_qt_audio_device


class LibmpvBackgroundError(RuntimeError):
    pass
...
class _DeckReader(threading.Thread):
    """Decode one file with bundled libmpv into the deck's bounded buffer."""
    ...
    def run(self):
        ...
        try:
            ...
            self.rendered_path = decode_audio_wav(
                deck.path, sample_rate=self.sample_rate, channels=CHANNELS,
                start_seconds=max(0.0, float(deck.start_seconds)), timeout=180,
            )
        except Exception:
            with deck.cond:
                deck.eof = True
            ...
            return
        try:
            with wave.open(self.rendered_path, "rb") as handle:
                while not self.stop_event.is_set():
                    data = handle.readframes(_READ_FRAMES)
                    if not data:
                        break
                    ...
                    if usable <= 0:
                        continue
                    block = (values[:usable].reshape(-1, CHANNELS).astype(np.float32)
                             / 32768.0)
                    with deck.cond:
                        while (not self.stop_event.is_set() and
                               deck.buffered_frames >= DECK_BUFFER_FRAMES):
                            deck.cond.wait(0.1)
            ...
            try:
                if self.rendered_path:
                    Path(self.rendered_path).unlink(missing_ok=True)
            except OSError:
                pass
...
class LibmpvBackgroundEngine:
    ...
    def __init__(
        ...
        self._feeder = None
        # Fail during construction rather than on the first recovery deck.
    ...
    def _create_sink(self):
        try:
            ...
            raise LibmpvBackgroundError(f"QAudioSink unavailable: {exc}") from exc
    ...
    def _make_deck(self, path: str, volume: float, norm_gain: float = 1.0,
        ...
        if not Path(path).exists():
            raise LibmpvBackgroundError(f"missing file: {path}")
        deck = _Deck(
            path=str(path),
            norm_gain=self._norm_factor(norm_gain),
            start_seconds=max(0.0, float(start_seconds)),
            duration_seconds=probe_duration_seconds(str(path)),
        )
    ...
    def load(self, path: str, paused: bool = True, volume: float | None = None):
        ...
        if volume is not None:
            ...
            self._master_ramp_frames = 0
        # Deck creation opens the decoded PCM source outside the lock so the
        # audio thread's mix_block never waits on offline libmpv work.
...
class _MixFeeder(_make_mix_feeder_base()):
    ...
    def __init__(self, engine: LibmpvBackgroundEngine):
        super().__init__()
        self.engine = engine
