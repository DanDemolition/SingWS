...
class RotationIdentityTests(unittest.TestCase):
    ...
    def test_waitlisted_phone_replacement_stays_waitlisted_when_slot_is_empty(self):
        app = make_app(self.singws)
        app.queue = [
            {
                "name": "Ada",
        ...
        attempted = []
        app.process_external_request = lambda incoming: attempted.append(incoming) or True

        app._reconcile_remote_requests([req])
