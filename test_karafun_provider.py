...
class KaraFunProviderTests(unittest.TestCase):
    ...
    def test_assisted_workflow_manages_the_macos_show_screen(self):
        ...
        self.assertIn("vw.showMinimized()", source)
        ...
        self.assertIn('perform action "AXPress" of bestButton', source)
        self.assertIn("click at {bestButtonX, bestButtonY}", source)
        self.assertIn("click elem", source)
        self.assertIn("FAST_READY", source)
        self.assertIn("show-screen fast handoff ready fullscreen forced", source)
        self.assertIn('set value of attribute "AXFullScreen" of outputWindow to true', source)
        self.assertIn('return "MISS|geometry"', source)
        ...
        self.assertIn("Dual Renderer fullscreen confirmed", source)
        ...
        self.assertIn("skipped KaraFun Dual Renderer minimize during active playback", restore)
    ...
    def test_location_permission_is_once_and_karafun_restore_has_fallback(self):
        ...
        self.assertIn("QTimer.singleShot(1400, _restore_singws)", restore)
    ...
    def test_external_start_wires_exact_search_play_and_adjustment_worker(self):
        ...
        self.assertIn("name of elem is {safe_title_literal}", search_helper)
        self.assertIn('if (role of elem is "AXStaticText") and (name of elem is {safe_title_literal}) then', search_helper)
        ...
        self.assertIn('entry["duration"] = int(selected_duration)', worker)
